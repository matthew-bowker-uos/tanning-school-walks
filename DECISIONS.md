...
## DEC-022 — London added to route-exposure analysis; NE re-run through cross-regional pipeline for methodological parity
**Date:** 2026-05-03
**Reason:** Two consistency issues resolved simultaneously:

1. **NE methodology inconsistency.** The original NE route buffer used OA-level origin centroids (~8,627 routes) and old 2 km/5 km caps. All cross-regional regions used LSOA-level PWCs (~1,600 routes) and the DEC-017 empirical caps (1.6 km/3.2 km). NE has now been re-run through `region_route_pipeline.py` with the same settings as the other four cross-regional regions: LSOA PWCs, 1.6/3.2 km caps, origin-IMD, age-cohort weights, cluster-bootstrap by LAD. Results: NE primary RII=0.705 (0.366–1.225), secondary RII=0.962 (0.559–1.910). These replace the earlier NE estimates in all primary outputs. The old OA-based NE estimates are retained in `data/processed/route_buffer_50m_20260428.gpkg` for sensitivity reference.

2. **London added.** London was included in the cross-regional density analysis (T13) and verification CSV but not the route-RII analysis. The rigorously correct analysis includes London: (a) the density result (flat Q1/Q5 ratio) is a genuine empirical finding that merits a route-RII companion; (b) London's verified false-positive rates are known (455/800 = 56.9% True); (c) omitting London from the route analysis while including it in the density comparison would be methodologically asymmetric. London has been added to `region_route_pipeline.py` with bbox (-0.566, 51.272, 0.391, 51.704) and all 33 GLA borough LAD codes (E09000001–E09000033). Walk network built from the same GB PBF, same 1.6/3.2 km caps.

**How to apply:** T14 / T14_verified / T15_verified now contain 6 regions. Manuscript introduction should note London explicitly alongside the five non-London regions. The flat London route-RII (expected given the flat density pattern) is an important null comparator: it shows that the positive gradient seen in NE/SW is not a universal feature of English cities.
